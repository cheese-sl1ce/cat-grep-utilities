...
int ParseFlags(int argc, char* argv[], cat_flags* flags) {
  ...
  int opt;
  while (is_error == 0 && (opt = getopt_long(argc, argv, "beEvnstT",
                                             long_options, NULL)) != -1) {
    switch (opt) {
      case 'b':
        flags->b = 1;
...
void HandleLineNumbering(int ch, int prev_ch, int* line_count,
                         cat_flags flags) {
  if (prev_ch == '\n') {
    if ((flags.b && ch != '\n') || (flags.n && flags.b == 0)) {
      (*line_count)++;
...
void ProcessFile(FILE* fp, cat_flags flags, int* line_count, int* prev_ch,
...
int main(int argc, char* argv[]) {
  ...
  if (return_code == 0) {
    ...
    while (i < argc) {
      ...
      if (fp != NULL) {
        ProcessFile(fp, flags, &line_count, &prev_ch, &empty_line_count);
        fclose(fp);
      } else {
